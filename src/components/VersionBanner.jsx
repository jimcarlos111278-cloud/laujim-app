      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 p-3 bg-yellow-50 dark:bg-yellow-900/80 border-t border-yellow-200 dark:border-yellow-700 shadow-lg">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-3">
          <p className="text-sm text-yellow-800 dark:text-yellow-200">
            ...
            <span className="text-yellow-600 ml-1">— recarga para actualizar</span>
          </p>
          <div className="flex gap-2 shrink-0">
            <button onClick={() => setDismissed(true)} className="px-3 py-1.5 text-xs rounded-lg border border-yellow-300 text-yellow-700 hover:bg-yellow-100">Cerrar</button>
            <button onClick={() => window.location.reload()} className="px-3 py-1.5 text-xs rounded-lg bg-blue-600 text-white hover:bg-blue-700">Actualizar</button>
          </div>
        </div>
      </div>
    );
