...
export default function Settings() {
  ...
  async function updateBackgroundNotificationPreference(key, value) {
    ...
    if (key === 'payments' && value === false) {
      await cancelAllNotifications();
    } else if (key === 'payments' && value === true && next.enabled) {
      const apartments = await api.apartments.toArray();
      await schedulePaymentReminders(apartments);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div data-settings-panel="notifications" className="bg-white dark:bg-gray-800 rounded-xl border border-emerald-200 dark:border-emerald-900/60 p-5 col-span-1 lg:col-span-2">
          ...
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
            {[
              ['backgroundEnabled', 'Servicio en segundo plano', 'Mantiene la revisión activa; muestra un único aviso silencioso'],
              ['whatsapp', 'WhatsApp', 'Mensajes, fotos, videos y notas de voz'],
              ['scraper', 'Scraper', 'Solo fallos o resultados parciales; los éxitos no avisan'],
              ['facebook', 'Facebook Marketplace', 'Solo publicaciones confirmadas; los errores no avisan'],
              ['payments', 'Pagos de canon por SMS', 'Transferencias recibidas para identificar y registrar el canon; no servicios públicos'],
        ...
        <div data-settings-panel="general" className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <input type="text" readOnly value={`${getPublicBaseUrl()}/publico`} className="flex-1 text-sm text-gray-700 dark:text-gray-200 bg-transparent outline-none" onClick={e => e.target.select()} />
            <button onClick={() => { navigator.clipboard.writeText(`${getPublicBaseUrl()}/publico`); }} className="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shrink-0">Copiar</button>
          </div>
        </div>
        ...
        <div data-settings-panel="whatsapp" className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Números que pueden consultar el reporte de cobros, validar pagos y recibir el recordatorio administrativo de los apartamentos que vencen ese día a las 8:00 a. m. CDT. Formato: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">57XXXXXXXXXX</code> (código de país + número, sin + ni espacios).</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">La plantilla aprobada por Meta debe llamarse <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">recordatorio_admin_cobros</code>, salvo que configures otro nombre en Render.</p>
