...
import { getBase, photoUrl, retryPhotoSource } from '../utils/config';
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-6">
          {apartments.map(apt => {
            ...
            const aptPhotoUrls = aptPhotos.map(photoUrl).filter(Boolean);
            return (
              <div key={apt.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                {aptPhotoUrls.length > 0 && (
                  <div className="relative bg-gray-100">
                    <img src={aptPhotoUrls[0] || ''} alt={apt.name} className="w-full h-56 object-cover" onError={e => { if (!retryPhotoSource(e, aptPhotos[0])) e.currentTarget.style.display = 'none'; }} />
                    {aptPhotoUrls.length > 1 && (
                      <div className="absolute top-3 right-3 flex gap-1">
                        {aptPhotoUrls.slice(0, 3).map((_, i) => (
                          <button key={i} onClick={() => setGallery({ photos: aptPhotoUrls, idx: i })} className="p-1.5 bg-black/40 text-white rounded-lg hover:bg-black/60 transition-colors text-xs">
                            <Image className="w-3.5 h-3.5" />
                          </button>
                        ))}
                        {aptPhotoUrls.length > 3 && (
                          <span className="p-1.5 bg-black/40 text-white rounded-lg text-xs">+{aptPhotoUrls.length - 3}</span>
                        )}
                      </div>
                    )}
                  </div>
      ...
      {gallery && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setGallery(null)}>
          <div className="relative max-w-4xl max-h-[90vh] flex flex-col items-center" onClick={e => e.stopPropagation()}>
            ...
            <img src={gallery.photos[gallery.idx] || ''} alt="" className="max-w-full max-h-[80vh] object-contain rounded-lg" onError={e => { if (!retryPhotoSource(e, gallery.photos[gallery.idx])) e.currentTarget.style.display = 'none'; }} />
