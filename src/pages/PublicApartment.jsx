        ...
        <section className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          {imageUrls.length > 0 ? (
            <button type="button" onClick={() => setGalleryIndex(0)} className="block w-full relative bg-gray-100 text-left">
              <img src={imageUrls[0]} alt={`Apartamento ${apartment.name}`} className="w-full h-72 object-cover" onError={e => { if (!retryPhotoSource(e, photos[0])) e.currentTarget.style.display = 'none'; }} />
      ...
      {galleryIndex !== null && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setGalleryIndex(null)}>
          <div className="relative max-w-5xl w-full" onClick={event => event.stopPropagation()}>
            ...
            <img src={imageUrls[galleryIndex]} alt={`Apartamento ${apartment.name}`} className="max-h-[80vh] max-w-full mx-auto rounded-lg object-contain" onError={e => { if (!retryPhotoSource(e, photos[galleryIndex])) e.currentTarget.style.display = 'none'; }} />
