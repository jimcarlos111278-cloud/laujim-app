...
//   1. Custom (localStorage.apt_server_url) → Settings
...
1. Si `localStorage.apt_server_url` existe (configurado en Settings → Conexión al Servidor): se usa ese valor + `/api`
...
4. Si es navegador normal: usa `window.location.origin` + `/api`
...
|-----------|-------|-------|
| Puerto | `1011` | Fijo, no configurable por env |
| Auth | Header `x-auth-token: laujim laujim` | Excluye `/api/login` y `/api/version` |
| JSON body limit | `50mb` | `express.json({ limit: '50mb' })` |
| File upload limit | `20MB` | Multer `limits.fileSize` |
| DB persistencia | `data/database.json` | Load al inicio, save tras cada mutación |
| Seed data | 12 apartments, 2 users | Generado automáticamente si el archivo no existe |
...
3. Auth middleware — valida `x-auth-token` en todas las rutas excepto `/api/login` y `/api/version`
...
**Método de auth:** El frontend envía POST `/api/login` con `{ token: 'laujim laujim' }`. El servidor responde `{ authenticated: true, role: 'owner', name: 'Propietario' }`. No hay sesiones ni JWT; el token se envía en cada request vía header.
...
android {
    ...
    defaultConfig {
        ...
        versionName = "1.0." + new SimpleDateFormat("yyyyMMdd-HHmm").format(new Date())
    }
}
```
...
// Incrementa patch automáticamente basado en version.json anterior
```

### `scripts/copy-apk.js` — Post-build APK

Copia `android/app/build/outputs/apk/debug/app-debug.apk` → `dist/app-debug.apk`.
...
### `src/db/database.js` — Dexie Schema v4
...
```js
new Dexie('ApartmentManager').version(4).stores({
...
**Seed data:** 2 usuarios (`admin/admin123` owner, `invitado/invitado123` guest) y 12 apartamentos (101 Casa, 102 Aparta Estudio, 201-404).
...
Colecciones expuestas: `users`, `apartments`, `tenants`, `contracts`, `payments`, `expenses`, `utilityPayments`, `vacancies`, `familyMembers`, `photos`.

Operaciones especiales:
- `uploadPhoto(file, apartmentId)` — sube foto vía FormData, la agrega a IndexedDB y encola
...
**Colecciones sincronizadas (11):** `users`, `apartments`, `tenants`, `contracts`, `payments`, `expenses`, `utilityPayments`, `vacancies`, `familyMembers`, `settings`, `photos`.
...
- En **APK**: compara `version.json` local vs servidor (por patch). Si servidor > local, muestra banner para descargar nueva APK.
...
| Módulo | Descripción |
|--------|-------------|
| **Dashboard** | Estadísticas generales: ocupación, ingresos mensuales, pagos pendientes, comparativa mensual, próximos pagos con cuenta regresiva, alerta de apartamentos vacantes |
...
| **Detalle de Apartamento** | Fotos (subida/eliminación), inquilino actual, historial de contratos, miembros de familia, historial de pagos y gastos, registro de vacancias, exportar PDF, compartir WhatsApp |
...
| **Configuración** | URL del servidor (para APK), sincronización manual (push/pull), estado de cambios pendientes, notificaciones del navegador, generar página HTML de aptos disponibles (compartir), descargar APK, enlaces a servicios públicos de Barranquilla |
...
│   ├── db/
│   │   └── database.js         # Dexie schema v4 + seed data (12 aptos, 2 usuarios)
│   ├── components/
...
│   │   # cleartext traffic = true, INTERNET, ACCESS_NETWORK_STATE
│   ├── app/build.gradle
...
│
├── scripts/
│   ├── generate-version.js     # Genera version.json al build (incrementa patch)
│   ├── copy-apk.js             # Copia APK de android/ a dist/
...
| POST | `/login` | Login con token | No | `{ token: string }` |
...
**Colecciones disponibles (11):** `apartments`, `tenants`, `contracts`, `payments`, `expenses`, `utilityPayments`, `vacancies`, `familyMembers`, `users`, `settings`, `photos`
...
| `/settings` | `Settings.jsx` | Configuración, sync, APK, compartir, notificaciones |
...
101 Casa, 102 Aparta Estudio, 201, 202, 203, 301, 302, 303, 401, 402, 403, 404 — 4 pisos, 3 aptos por piso (piso 4 tiene 4). Todos con `paymentDueDay: 5`, `status: 'vacant'`.
...
```bash
npm run build
# Genera dist/ con version.json (incrementa patch automáticamente)
```
...
4. **Sin conexión**: el app funciona 100% offline con datos locales
5. **Con conexión**: replica automáticamente al servidor
6. **Detección de cambios**: `syncAllWithChanges()` compara pagos antes/después via JSON.stringify
7. **Desde Settings**: botón "Sincronizar Ahora" (push+pull), botón "Pull" (solo traer datos)
...
1. **`usesCleartextTraffic`**: Android 9+ bloquea HTTP por defecto. Ya agregado en `AndroidManifest.xml`:
   ...
   ```
2. **URL del servidor**: Configurar en Settings → Conexión al Servidor. Por defecto `http://192.168.1.21:1011`. Guarda en `localStorage.apt_server_url`.
...
5. **`allowNavigation`**: Capacitor 8 requiere whitelist de hosts. Configurado en `capacitor.config.json` con rangos `192.168.1.*`, `192.168.0.*`, `10.0.2.*`, `localhost`, `127.0.0.1`.
6. **`window.Capacitor` detection**: `config.js` usa `window.Capacitor` para detectar APK y usar `DEFAULT_SERVER` en lugar de `window.location.origin` (que apunta al servidor interno de Capacitor).
