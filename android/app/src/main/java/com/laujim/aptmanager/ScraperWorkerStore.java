...
final class ScraperWorkerStore {
    ...
    static void configure(
        ...
        prefs(context).edit()
            ...
            .putString("backupServerUrl", normalizeServer(backupServerUrl))
            .putString("activeServerUrl", normalizeServer(serverUrl))
    ...
    static String backupServerUrl(Context context) {
        String configured = normalizeServer(prefs(context).getString("backupServerUrl", ""));
        return configured.isEmpty() ? DEFAULT_BACKUP_SERVER : configured;
    }
    static String activeServerUrl(Context context) {
        return normalizeServer(prefs(context).getString("activeServerUrl", ""));
    }
    static void setActiveServerUrl(Context context, String value) {
        String normalized = normalizeServer(value);
        if (!normalized.isEmpty()) prefs(context).edit().putString("activeServerUrl", normalized).apply();
    }
    ...
    static String defaultPrimaryServer() { return DEFAULT_PRIMARY_SERVER; }
    static String defaultBackupServer() { return DEFAULT_BACKUP_SERVER; }
}
