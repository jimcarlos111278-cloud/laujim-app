...
final class ScraperWorkerDispatcher {
    ...
    static synchronized boolean dispatchProvider(Context context, String source, String provider, boolean resetSession, boolean force) {
        String normalized = provider == null ? "" : provider.trim().toLowerCase();
        if (!(normalized.equals("air-e") || normalized.equals("water") || normalized.equals("gas") || normalized.matches("gas-\\d+"))) {
            return false;
        }
        return dispatchInternal(
            ...
            source,
            force,
            ScraperWorkerService.ACTION_RUN_PROVIDER,
            null,
            normalized,
            resetSession
        );
    ...
    private static boolean dispatchInternal(Context context, String source, boolean force, String action, String accountId) {
        return dispatchInternal(context, source, force, action, accountId, null, false);
    }

    private static boolean dispatchInternal(Context context, String source, boolean force, String action, String accountId, String provider, boolean resetSession) {
        ...
        if (provider != null) service.putExtra(ScraperWorkerService.EXTRA_PROVIDER, provider);
        if (resetSession) service.putExtra(ScraperWorkerService.EXTRA_RESET_PROVIDER_SESSION, true);
        try {
