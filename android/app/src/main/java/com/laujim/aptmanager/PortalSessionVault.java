...
final class PortalSessionVault {
    ...
    private static final String[] AIR_ORIGINS = {
        "https://portal.air-e.com/"
    };
    private static final String[] WATER_ORIGINS = {
    ...
    /** Clears one provider's expired browser identity without touching the
     * authenticated sessions of the other utility portals. */
    static void clearProviderBrowserState(Context context, String provider) {
        String normalized = normalize(provider);
        clearProvider(context, normalized);
        String[] origins = originsFor(normalized);
        clearCookies(origins);
        try {
            WebStorage storage = WebStorage.getInstance();
            for (String origin : origins) storage.deleteOrigin(origin.substring(0, origin.length() - 1));
    ...
    static void clearGasCookies() {
        clearCookies(GAS_ORIGINS);
    }

    private static void clearCookies(String[] origins) {
        try {
            CookieManager cookies = CookieManager.getInstance();
    ...
    private static String[] originsFor(String provider) {
        String base = baseProvider(provider);
        if ("water".equals(base)) return WATER_ORIGINS;
        if ("gas".equals(base)) return GAS_ORIGINS;
        return AIR_ORIGINS;
    }
