...
(function () {
  ...
  function installmentProgress(value) {
    ...
    const raw = field(value, [
      // These are the labels used by the rendered provider cards. Keep them
      // before generic `cuotas`/`installments` fields because those can refer
      // to an invoice count rather than the financing plan.
      'cuotasFacturadas', 'cuotasPagadas', 'avanceConvenio', 'avanceFinanciacion',
      'quotaProgress', 'installmentProgress', 'billedQuotas', 'paidQuotas',
      'numberOfQuotas', 'installments', 'installmentNumber', 'cuotas', 'quotas',
      'installmentStatus', 'quotaStatus', 'estadoCuotas', 'avanceCuotas',
      'installmentsProgress', 'paymentPlanProgress',
    ]);
    const match = String(raw == null ? '' : raw).match(/(\d+)\s*(?:de|\/|of)\s*(\d+)/i);
    if (match) {
      current = Number(match[1]);
      total = Number(match[2]);
    }
    const explicitCurrent = field(value, [
      'cuotaActual', 'numeroCuota', 'cuotasPagadas', 'cuotasFacturadas',
      'currentQuota', 'currentInstallment', 'installmentNumber', 'quotaNumber',
      'paidQuotas', 'billedQuotas', 'quotasPaid',
      'installmentsPaid', 'paidInstallments', 'completedInstallments',
      'cuotasPagadas', 'numeroCuotasPagadas', 'cantidadCuotasPagadas',
      'currentInstallmentNumber', 'currentPaymentNumber', 'billedInstallments',
    ]);
    const explicitTotal = field(value, [
      'cuotasTotales', 'numeroTotalCuotas', 'totalQuotas', 'numberOfQuotas',
      'totalInstallments', 'installmentCount', 'quotasTotal', 'quotas',
      'installmentsTotal', 'paymentCount', 'planTerm', 'term', 'plazo',
      'cantidadCuotas', 'totalPayments',
    ]);
    const explicitPending = field(value, [
      'pendingQuotas', 'remainingQuotas', 'pendingInstallments', 'remainingInstallments',
      'cuotasPendientes', 'cuotasRestantes', 'installmentsRemaining',
    ]);
    if (current === null && /^\d+$/.test(String(explicitCurrent ?? '').trim())) current = Number(explicitCurrent);
    if (total === null && /^\d+$/.test(String(explicitTotal ?? '').trim())) total = Number(explicitTotal);
    if (current === null && total !== null && /^\d+$/.test(String(explicitPending ?? '').trim())) {
      current = Math.max(0, total - Number(explicitPending));
    }
    if (current === null || total === null) {
      const text = Object.entries(value && typeof value === 'object' ? value : {})
        .filter(([, child]) => child == null || typeof child !== 'object')
        .map(([key, child]) => `${key}: ${String(child ?? '')}`)
        .join(' ');
      const textMatch = text.match(/(?:cuotas?|installments?|pagos?)\D{0,24}(\d+)\s*(?:de|\/|of)\s*(\d+)/i)
        || text.match(/(\d+)\s*(?:de|\/|of)\s*(\d+)\D{0,24}(?:cuotas?|installments?)/i);
      if (textMatch) {
        if (current === null) current = Number(textMatch[1]);
        if (total === null) total = Number(textMatch[2]);
      }
    }
    ...
    return {
      cuotaActual: Number.isFinite(current) ? current : null,
      cuotasTotales: Number.isFinite(total) ? total : null,
    };
  }
  ...
  function invoiceStats(invoices) {
    ...
    const now = Date.now();
    const unpaid = rows.filter(invoice => !recordIsPaid(invoice));
    const explicitOverdue = rows.map(invoice => amountFromFields(invoice, [
      'expiredDocuments', 'overdueDocuments', 'overdueInvoices', 'pastDueInvoices',
      'facturasVencidas', 'documentosVencidos',
    ])).find(value => value !== null);
    let hasOverdueEvidence = explicitOverdue !== null;
    const overdue = unpaid.filter(invoice => {
      const status = clean(field(invoice, ['status', 'state', 'paymentStatus', 'invoiceStatus']));
      if (status) hasOverdueEvidence = true;
      if (/vencid|overdue|mora|past due/.test(status)) return true;
      const rawDue = field(invoice, [
        'expirationDate', 'dueDate', 'paymentDueDate', 'fechaVencimiento',
        'fechaLimitePago', 'fechaPagoOportuno', 'dt_FechaVencimiento',
      ]);
      const due = rawDue ? new Date(rawDue) : null;
      if (due && Number.isFinite(due.getTime())) hasOverdueEvidence = true;
      return due && due.getTime() < now;
    });
    return {
      facturasTotales: rows.length,
      facturasPendientes: unpaid.length,
      facturasVencidas: explicitOverdue !== null
        ? Math.max(0, Math.round(explicitOverdue))
        : hasOverdueEvidence ? overdue.length : null,
    };
  }

  function financingSummary(payload) {
    // Never search for a financing amount in the whole response without
    // context. The providers return current invoices, account totals and
    // financing cards in the same payload; a recursive first-match parser
    // previously copied one of those unrelated totals into “Convenio”.
    const records = objectRecords(payload);
    const normalizeKey = value => normalizedFieldKey(value);
    ...
    const strongAmountKeys = [
      ...
      'saldoPendienteFinanciado', 'saldoPendienteConvenio',
    ];
    const genericFinanceKey = ['saldoPorFacturar'];
    const quotaKeys = [
      'quotaValue', 'cuotaValue', 'cuotaFinanciada', 'installmentValue',
      'monthlyQuota', 'valorCuota', 'valorCuotaFinanciada', 'cuotaMensual',
    ];
    const nextPaymentKeys = [
      'nextPayment', 'proximoPago', 'nextInstallment', 'cuotaProxima',
      'nextPaymentValue', 'valorProximoPago', 'siguientePago',
    ];
    const gasBalanceKeys = [
      'pendingGasBalance', 'saldoPorFacturarGas', 'gasPendingBalance',
      'pendingGasAmount', 'saldoGas', 'saldoPorFacturarGasCOP',
    ];
    const financialBalanceKeys = [
      'pendingFinancialBalance', 'saldoPorFacturarServiciosFinancieros',
      'financialPendingBalance', 'pendingFinancialAmount', 'saldoFinanciero',
      'saldoPorFacturarFinancieroCOP',
    ];
    const financeWords = /financ|refinanc|diferid|acuerdo|conveni|plan\s+de\s+pago|brilla|debtfinanc|deferred/i;
    const hasDedicatedContext = record => {
      const text = textOf(record);
      const keys = Object.keys(record || {}).join(' ');
      return financeWords.test(text) || /financ|defer|conveni|acuerdo|saldoporfacturar|saldofinanciado|deudafinanciada|saldo(?:pendiente)?financiado/i.test(keys);
    };
    const across = (names, predicate) => {
      for (const record of records) {
        if (predicate && !predicate(record)) continue;
        const value = directAmount(record, names);
        if (value !== null) return value;
      }
      return null;
    };
    const explicit = across(strongAmountKeys, record => hasDedicatedContext(record));
    // `saldoPorFacturar` is only accepted when the same object also carries a
    // quota or an explicit financing marker. This prevents a current-invoice
    // balance with a coincidental generic key from becoming a convention.
    const genericExplicit = across(genericFinanceKey, record =>
      hasDedicatedContext(record) || directAmount(record, quotaKeys) !== null
    );
    const explicitFinanced = explicit ?? genericExplicit;
    const explicitQuota = across(quotaKeys);
    const nextPayment = across(nextPaymentKeys);
    const pendingGas = across(gasBalanceKeys);
    const pendingFinancial = across(financialBalanceKeys);
    const componentPresent = pendingGas !== null || pendingFinancial !== null;
    const rows = [];
    records.forEach(record => {
      const text = textOf(record);
      const hasHint = hasDedicatedContext(record) || /cuota|installment/i.test(text) && financeWords.test(text);
      if (!hasHint) return;
      // A wrapper object is not a financing row unless it has a descriptive
      // field or is itself the only row passed to the parser.
      const hasDescription = directField(record, [
      ...
      const quota = directAmount(record, quotaKeys.concat(nextPaymentKeys));
      ...
      rows.push({
        ...
        producto: product ? String(product).replace(/\s+/g, ' ').trim().slice(0, 120) : null,
        saldoCOP: amount,
        cuotaCOP: quota,
        cuotas: directField(record, ['cuotasFacturadas', 'cuotasPagadas', 'billedQuotas', 'paidQuotas', 'quotas', 'numberOfQuotas']) ?? null,
        cuotaActual: progress.cuotaActual,
    ...
    return {
      ...
      cuotaActual: progress?.cuotaActual ?? null,
      cuotasTotales: progress?.cuotasTotales ?? null,
      proximoPagoCOP: nextPayment,
      saldoPorFacturarGasCOP: pendingGas,
      saldoPorFacturarFinancieroCOP: pendingFinancial,
      financingConfirmed,
      financeValidation: financingConfirmed ? 'confirmed' : 'absent',
      financingSource: componentPresent ? 'component_balances' : explicitFinanced !== null ? 'financing_fields' : rows.length ? 'financing_rows' : null,
  ...
  function tripleAInvoiceSummary(invoices) {
    ...
    const stats = invoiceStats(listItems);
    return {
      ...
      facturasTotales: stats.facturasTotales,
      facturasPendientes: stats.facturasPendientes,
      facturasVencidas: stats.facturasVencidas,
  ...
  function resultBase(provider, service, target, extra) {
    ...
    return Object.assign({
      ...
      financeValidation: 'absent',
      financingSource: null,
      financiacion: [],
      proximoPagoCOP: null,
      saldoPorFacturarGasCOP: null,
      saldoPorFacturarFinancieroCOP: null,
      facturas: [],
      debtSource: null,
  ...
  // A protected route can remain visually mounted after its cookie or bearer
  // session has expired. In that state there is no password input to detect,
  // so report a recoverable login outcome and let the native worker reload the
  // portal before evaluating the same provider again.
  function recoverableSessionOutcome(provider, config, stage, message, extra) {
    if (hasConfiguredCredentials(config)) {
      return needsLogin(provider, message || `${providerLabel(provider)} no confirmó una sesión válida.`, Object.assign({
        stage,
    ...
    return Object.assign({
      ...
      provider,
      stage,
      message: message || `${providerLabel(provider)} no confirmó una sesión válida.`,
      results: [],
    }, extra || {});
  }
  ...
  function detailedUiDebt(text, fallbackAmount) {
    ...
    // The navigation can contain a "Deuda financiada" label even when the
    // selected account has no agreement. Require a financing-only field before
    // treating that label as a real card; otherwise the current card's total
    // was incorrectly copied into Convenio.
    const financingMarkers = /saldo\s+por\s+facturar|saldo\s+pendiente|n[uú]mero\s+de\s+financiaci[oó]n|financiaci[oó]n\s+productos?|valor\s+(?:de\s+la\s+)?cuota|cuotas?\s*(?:facturadas?|pendientes?)|saldo\s+inicial/i;
    const financingText = financingSection && financingMarkers.test(financingSection)
      ? financingSection
      : '';
    ...
    // Gascaribe shows the financed balance in two separate fields (Gas and
    // Servicios financieros). Reading the first “Saldo total” used to pick
    // the $0 financial component or a stale card. Prefer the component sum.
    const convenio = hasGasComponents
      ? (gasBalance || 0) + (financialBalance || 0)
      : uiAmountForLabel(financingText, /saldo\s+pendiente|saldo\s+total|valor\s+total/i)
        ?? uiAmountNearHeading(financingText, /deuda\s+(?:financiada|diferida)|convenio|financiaci[oó]n/i);
    const lines = uiLines(financingText);
    const amountAt = (index, matcher) => {
      const line = lines[index] || '';
      const labelMatch = matcher.exec(line);
      const tail = labelMatch ? line.slice(labelMatch.index + labelMatch[0].length) : '';
      const inline = tail.match(/\$\s*([0-9][0-9.,]*)/);
      if (inline) return parseAmount(inline[1]);
      for (const next of lines.slice(index + 1, index + 4)) {
        const match = next.match(/\$\s*([0-9][0-9.,]*)/);
        if (match) return parseAmount(match[1]);
      }
    ...
    const rowIndexes = lines.map((line, index) => /^valor\s+total\b/i.test(line) ? index : -1).filter(index => index >= 0);
    const financingRows = [];
    rowIndexes.forEach((index, rowIndex) => {
      const end = rowIndexes[rowIndex + 1] ?? lines.length;
      const segment = lines.slice(index, end).join(' ');
      const amount = amountAt(index, /^(?:valor\s+total|saldo\s+por\s+facturar)\b/i);
      const quota = uiAmountForLabel(segment, /valor\s+(?:de\s+la\s+)?cuota\b/i);
      const progressMatch = segment.match(/(?:cuotas?\s+facturadas?|cuotas?|pagos?)\s*[:#-]?\s*(\d+\s*(?:de|\/)\s*\d+)/i);
      const progress = installmentProgress({ cuotasFacturadas: progressMatch?.[1] || null });
      const labelLine = lines.slice(Math.max(0, index - 3), end).find(line => /^(?:producto|concepto|financiaci[oó]n productos?)\b/i.test(line));
      const labelIndex = labelLine ? lines.indexOf(labelLine, Math.max(0, index - 3)) : -1;
      const label = labelLine
        ? String(lines[labelIndex + 1] || labelLine).replace(/^producto\s*[:#-]?\s*/i, '').trim()
        : null;
      const number = segment.match(/(?:no\s*[•·\-:]?|n[uú]mero\s+de\s+financiaci[oó]n)\s*([0-9]{4,})/i)?.[1] || null;
      const saldoInicial = uiAmountForLabel(segment, /saldo\s+inicial\b/i);
      const fechaInicio = (() => {
        const position = lines.findIndex((line, lineIndex) => lineIndex >= index && lineIndex < end && /^(?:fecha\s+de\s+inicio|inicio)\b/i.test(line));
        return position >= 0 ? String(lines[position + 1] || '').trim() || null : null;
      })();
      if (amount === null && quota === null) return;
      financingRows.push({
        concepto: label || 'Financiación',
        producto: label || null,
        saldoCOP: amount,
        cuotaCOP: quota,
        cuotaActual: progress.cuotaActual,
        cuotasTotales: progress.cuotasTotales,
        cuotas: progressMatch?.[1] || null,
        numero: number,
        saldoInicialCOP: saldoInicial,
        fechaInicio,
      });
    });
    // Triple A's deferred-debt screen normally has one card with a single
    // "Valor total" and "Saldo pendiente". If the mobile WebView flattens the
    // card and the row marker is lost, retain that card as one financing row so
    // the quota/progress data is not discarded.
    const fallbackQuota = uiAmountForLabel(financingText, /valor\s+(?:de\s+la\s+)?cuota\b/i);
    if (financingText && !financingRows.length && (convenio !== null || fallbackQuota !== null)) {
      const progressMatch = financingText.match(/(?:cuotas?\s+facturadas?|cuotas?|pagos?)\s*[:#-]?\s*(\d+\s*(?:de|\/)\s*\d+)/i);
      const progress = installmentProgress({ cuotasFacturadas: progressMatch?.[1] || null });
      const lineAfter = matcher => {
        const position = lines.findIndex(line => matcher.test(line));
        return position >= 0 ? String(lines[position + 1] || '').trim() || null : null;
      };
      financingRows.push({
        concepto: lineAfter(/^descripci[oó]n\b/i) || lineAfter(/^producto\b/i) || 'Financiación',
        producto: lineAfter(/^producto\b/i),
        saldoCOP: convenio,
        cuotaCOP: fallbackQuota,
        cuotaActual: progress.cuotaActual,
        cuotasTotales: progress.cuotasTotales,
        cuotas: progressMatch?.[1] || null,
        numero: financingText.match(/(?:n[uú]mero\s+de\s+financiaci[oó]n|no\s*[•·\-:])\s*([0-9]{4,})/i)?.[1] || null,
        saldoInicialCOP: uiAmountForLabel(financingText, /saldo\s+inicial\b/i),
        fechaInicio: lineAfter(/^fecha\s+de\s+inicio\b/i),
      });
    }
    const progress = financingRows.find(row => row.cuotaActual !== null || row.cuotasTotales !== null)
      || installmentProgress({ cuotasFacturadas: financingText.match(/(?:cuotas?\s+facturadas?|cuotas?|pagos?)\s*[:#-]?\s*(\d+\s*(?:de|\/)\s*\d+)/i)?.[1] || null });
    const nextPaymentCOP = uiAmountForLabel(financingText, /siguiente\s+pago|pr[oó]ximo\s+pago/i);
    const rowQuotaValues = financingRows.map(row => row.cuotaCOP).filter(value => value !== null);
    const cuotaFinanciadaCOP = rowQuotaValues.length
      ? rowQuotaValues.reduce((sum, value) => sum + value, 0)
      : uiAmountForLabel(financingText, /valor\s+de\s+la\s+cuota|valor\s+cuota/i);
    const pendingMatch = currentSection.match(/facturas?\s+pendientes?\s*[:#-]?\s*(\d+)/i);
    const overdueMatch = source.match(/(?:facturas?|documentos?)\s+vencid[oa]s?\s*[:#-]?\s*(\d+)/i);
    ...
    return {
      ...
      cuotaFinanciadaCOP: cuotaFinanciadaCOP ?? nextPaymentCOP,
      cuotaActual: progress.cuotaActual,
      cuotasTotales: progress.cuotasTotales,
      proximoPagoCOP: nextPaymentCOP,
      saldoPorFacturarGasCOP: gasBalance,
      saldoPorFacturarFinancieroCOP: financialBalance,
      facturasPendientes: pendingMatch ? Number(pendingMatch[1]) : null,
      facturasVencidas: overdueMatch ? Number(overdueMatch[1]) : null,
      financingConfirmed: Boolean(financingText && (convenio !== null || financingRows.length)),
      financeValidation: financingText && (convenio !== null || financingRows.length) ? 'confirmed' : 'absent',
      financingSource: hasGasComponents ? 'component_balances' : financingRows.length ? 'financing_rows' : financingText ? 'financing_card' : null,
      financiacion: financingRows.slice(0, 20),
    };
  }
  ...
  function waterPolicySelectorTrigger() {
    ...
    const arrowSelector = '.material-icons, [class*="arrow-down"], [class*="ri-arrow-down"], [class*="chevron"], svg';
    const expandable = element => {
      ...
      return /^(combobox|listbox)$/.test(role)
        || ariaHasPopup != null
        || ariaExpanded != null
        || /select|dropdown|combobox|popover/.test(className);
    };
    const pairNodes = Array.from(document.querySelectorAll('button, [role="button"], [role="combobox"], div, span'))
      .filter(domAvailable)
      .map(element => ({
        element,
        text: uiText(element).replace(/\s+/g, ' ').trim(),
    ...
    for (const candidate of pairNodes) {
      let current = candidate.element;
      for (let depth = 0; current && depth < 7; depth += 1, current = current.parentElement) {
        if (!domAvailable(current)) break;
        const text = uiText(current).replace(/\s+/g, ' ').trim();
        if (!text || text.length >= 180 || !portalPair(text)) continue;
        if (expandable(current) || current.querySelector(arrowSelector)) return current;
      }
    }
    // Some builds use an unlabelled SVG chevron and put the click handler on
    // a plain div. Clicking the smallest policy text still bubbles to that
    // handler and is safer than depending on a CSS class name.
    return pairNodes[0]?.element || null;
  }
  ...
  function selectedWaterPolicyCode() {
    const trigger = waterPolicySelectorTrigger();
    const triggerPair = portalPair(trigger ? uiText(trigger) : '');
    if (triggerPair) return digits(triggerPair[2]);
  ...
  async function selectWaterPolicy(policy) {
    ...
    return Boolean(await waitForUi(() => {
      ...
      const text = uiBodyText();
      // Some MUI versions keep the option nodes mounted while they animate
      // out. The selected trigger and debt card are the reliable completion
      // signals; requiring the menu nodes to disappear caused false errors
      // after the amount was already visible.
  ...
  async function openWaterPayments() {
    ...
    return await waitForUi(() => {
      const state = loginState();
      if (state.password) return { login: true };
      if (state.challenge) return { verification: true };
      const text = uiBodyText();
  ...
  function parseWaterPaymentResult(text) {
    ...
    return {
      ...
      cuotaFinanciadaCOP: detailed.cuotaFinanciadaCOP,
      cuotaActual: detailed.cuotaActual,
      cuotasTotales: detailed.cuotasTotales,
      proximoPagoCOP: detailed.proximoPagoCOP,
      saldoPorFacturarGasCOP: detailed.saldoPorFacturarGasCOP,
      saldoPorFacturarFinancieroCOP: detailed.saldoPorFacturarFinancieroCOP,
      facturasPendientes: detailed.facturasPendientes,
      facturasVencidas: detailed.facturasVencidas,
      financingConfirmed: detailed.financingConfirmed,
      financeValidation: detailed.financeValidation,
      financingSource: detailed.financingSource,
      financiacion: detailed.financiacion || [],
      status,
      dueDate,
    };
  }
  ...
  function parseGasHomeResult(text) {
    ...
    return {
      ...
      total: amount,
      cuotaFinanciadaCOP: paidByText ? 0 : detailed.cuotaFinanciadaCOP,
      cuotaActual: paidByText ? null : detailed.cuotaActual,
      cuotasTotales: paidByText ? null : detailed.cuotasTotales,
      proximoPagoCOP: paidByText ? null : detailed.proximoPagoCOP,
      saldoPorFacturarGasCOP: paidByText ? null : detailed.saldoPorFacturarGasCOP,
      saldoPorFacturarFinancieroCOP: paidByText ? null : detailed.saldoPorFacturarFinancieroCOP,
      facturasPendientes: paidByText ? 0 : detailed.facturasPendientes,
      facturasVencidas: paidByText ? 0 : detailed.facturasVencidas,
      financingConfirmed: paidByText || detailed.financingConfirmed,
      financeValidation: paidByText ? 'confirmed' : detailed.financeValidation,
      financingSource: paidByText ? 'paid_card' : detailed.financingSource,
      financiacion: paidByText ? [] : (detailed.financiacion || []),
      amount,
  ...
  async function queryGasContract(contract) {
    ...
    if (parsed && (parsed.convenio == null || parsed.convenio === 0)) {
      ...
      if (debtLink) {
        ...
        let debtText = uiBodyText();
        // The debt screen keeps its own contract selector. It may retain the
        // previous apartment even after the home selector changed. Re-select
        // and verify the contract before accepting any financed balance.
        if (!new RegExp(escapeRegex(contract.code)).test(debtText)) {
          const debtSelector = currentGasContractButton();
          if (debtSelector) {
            debtSelector.click();
            const debtOption = await waitForUi(() => findVisibleUiElement(
              '[role="menuitem"], [role="option"], li, button',
              label => label.includes(contract.code) && label !== uiText(debtSelector),
            ), 10_000);
            if (debtOption) {
              debtOption.click();
              await waitForUi(() => new RegExp(escapeRegex(contract.code)).test(uiBodyText()) ? true : null, 10_000);
              debtText = uiBodyText();
            }
          }
        }
  ...
  async function runAirE(config) {
    ...
    for (let attempt = 0; attempt < 12 && !contract; attempt += 1) {
      ...
      if (!contract && loginState().password) return await attemptAutoLogin('air-e', config || {});
      if (!contract) await wait(1000);
    }
    if (!contract) return recoverableSessionOutcome('air-e', config, 'discover_contract', 'Air-e no confirmó la sesión ni mostró el contrato autenticado. Se intentará iniciar sesión nuevamente.', {
      pageUrl: location.href,
      pageTitle: document.title || '',
      resourceCount: (() => { try { return (performance.getEntriesByType('resource') || []).length; } catch { return 0; } })(),
    });
    ...
    const results = (config.apartments || []).filter(target => target.electricityPaymentCode).map(target => {
      ...
      const stats = invoiceStats(group.invoices);
      ...
      return resultBase('Air-e', 'electricity', target, {
        ...
        deudaMesCOP,
        // Air-e's second balance is “Estado de Cuenta” and its secondary
        // report field is explicitly “Facturas vencidas”, never Convenio.
        // Do not let generic financing-shaped fields from the invoice API
        // leak into the other providers' financing columns.
        deudaConveniosCOP: 0,
        deudaTotalCOP,
        ...
        financingSource: null,
        financiacion: [],
        facturas: rows.slice(0, 24).map(row => ({
          numero: field(row.invoice, ['invoiceNumber', 'invoiceId', 'factura', 'id', 'nr_Documento', 'cd_Documento']) || null,
          periodo: field(row.invoice, ['invoiceDate', 'billingPeriod', 'periodo', 'fechaFactura', 'ds_Periodo', 'cd_Periodo']) || null,
          vencimiento: field(row.invoice, ['expirationDate', 'dueDate', 'paymentDueDate', 'fechaVencimiento', 'dt_FechaVencimiento']) || null,
          valorCOP: row.month ?? amountFromFields(row.invoice, ['amt_Valor', 'valor', 'amt_DeudaTotal', 'amt_SaldoConsulta']),
          estado: field(row.invoice, ['cd_EstadosPagoDocumento', 'status', 'state', 'paymentStatus']) || null,
        })),
  ...
  async function fetchTripleASummary(subscription, token) {
    ...
    // explicit financing marker; otherwise the agreement is confirmed as $0.
    const debtFinancing = financingSummary(debtPayload);
    const financing = financingPayloads.length
      ? financingSummary(financingPayloads)
      : debtFinancing.financeValidation === 'confirmed' ? debtFinancing : financingSummary(null);
    ...
    const convenio = financingConfirmed
      ? (financing.financiadaCOP ?? 0)
      : currentTotal === null ? null : 0;
    ...
    return {
      ...
      facturasTotales: invoiceSummary.facturasTotales,
      facturasPendientes: invoiceSummary.facturasPendientes,
      facturasVencidas: invoiceSummary.facturasVencidas,
      ...
      cuotaActual: financing.cuotaActual,
      cuotasTotales: financing.cuotasTotales,
      proximoPagoCOP: financing.proximoPagoCOP,
      saldoPorFacturarGasCOP: financing.saldoPorFacturarGasCOP,
      saldoPorFacturarFinancieroCOP: financing.saldoPorFacturarFinancieroCOP,
      financingConfirmed,
  ...
  async function fetchGasDebtSummary(contractId, auth) {
    ...
    return {
      ...
      deudaConveniosCOP: convenio,
      financiadaCOP: convenio,
      cuotaFinanciadaCOP: financingConfirmed ? financing.cuotaFinanciadaCOP : null,
      cuotaActual: financing.cuotaActual,
      cuotasTotales: financing.cuotasTotales,
      proximoPagoCOP: financing.proximoPagoCOP,
      saldoPorFacturarGasCOP: financing.saldoPorFacturarGasCOP,
      ...
      financeValidation: financing.financeValidation,
      financingSource: financing.financingSource || null,
      financiacion: financingConfirmed ? financing.financiacion : [],
  ...
  async function runWater(config) {
    ...
    for (const subscription of subscriptions) {
      ...
      results.push(resultBase('Triple A', 'water', target, {
        ...
        facturasTotales: summary.facturasTotales ?? null,
        facturasPendientes: summary.facturasPendientes ?? summary.numFacturas ?? null,
        facturasVencidas: summary.facturasVencidas ?? null,
        ...
        cuotaActual: summary.cuotaActual ?? null,
        cuotasTotales: summary.cuotasTotales ?? null,
        proximoPagoCOP: summary.proximoPagoCOP ?? null,
        saldoPorFacturarGasCOP: summary.saldoPorFacturarGasCOP ?? null,
        saldoPorFacturarFinancieroCOP: summary.saldoPorFacturarFinancieroCOP ?? null,
        financingConfirmed: Boolean(summary.financingConfirmed),
        financeValidation: summary.financeValidation || (summary.financingConfirmed ? 'confirmed' : 'absent'),
        financingSource: summary.financingSource || null,
      ...
      if (!invoiceResponse || !invoiceResponse.ok) {
        ...
        if ([404, 422].includes(Number(invoiceResponse?.status))) {
          ...
          results.push(resultBase('Gases del Caribe', 'gas', target, {
            ...
            cuotaActual: debtSummary.cuotaActual ?? null,
            cuotasTotales: debtSummary.cuotasTotales ?? null,
            proximoPagoCOP: debtSummary.proximoPagoCOP ?? null,
            saldoPorFacturarGasCOP: debtSummary.saldoPorFacturarGasCOP ?? null,
            saldoPorFacturarFinancieroCOP: debtSummary.saldoPorFacturarFinancieroCOP ?? null,
            financingConfirmed: Boolean(debtSummary.financingConfirmed),
            financeValidation: debtSummary.financeValidation || (debtSummary.financingConfirmed ? 'confirmed' : 'absent'),
            financingSource: debtSummary.financingSource || null,
  ...
  async function runWaterUi(config) {
    ...
    await wait(1200);
    const debtHome = await openWaterPayments();
    if (debtHome?.login) return await attemptAutoLogin('water', config || {});
    if (debtHome?.verification) return { state: 'needs_verification', provider: 'water', stage: 'verification_required', message: 'Triple A requiere completar una verificación visible antes de consultar.', results: [] };
    if (!debtHome) {
      return recoverableSessionOutcome('water', config, 'open_debt_home', 'Triple A no confirmó la sesión ni mostró el resumen de deuda. Se intentará iniciar sesión nuevamente.', {
        url: location.href,
        title: document.title || '',
      });
    }
    ...
    if (!policies.length) {
      return recoverableSessionOutcome('water', config, 'parse_policies', 'Triple A no confirmó las pólizas en la sesión actual. Se recargará el portal y se intentará recuperar el acceso.', {
        policyCount: 0,
    ...
    for (const policy of policies) {
      ...
      for (const target of targets) results.push(resultBase('Triple A', 'water', target, {
          ...
          deudaConveniosCOP: parsed.financingConfirmed
            ? (parsed.convenio ?? 0)
            : ((parsed.total ?? parsed.amount) == null ? null : 0),
          ...
          facturasTotales: parsed.status === 'pending' ? 1 : 0,
          facturasPendientes: parsed.facturasPendientes ?? (parsed.status === 'pending' ? 1 : 0),
          facturasVencidas: parsed.facturasVencidas ?? null,
          cuotaFinanciadaCOP: parsed.cuotaFinanciadaCOP ?? null,
          cuotaActual: parsed.cuotaActual ?? null,
          cuotasTotales: parsed.cuotasTotales ?? null,
          proximoPagoCOP: parsed.proximoPagoCOP ?? null,
          saldoPorFacturarGasCOP: parsed.saldoPorFacturarGasCOP ?? null,
          saldoPorFacturarFinancieroCOP: parsed.saldoPorFacturarFinancieroCOP ?? null,
          financingConfirmed: Boolean(parsed.financingConfirmed),
          financeValidation: parsed.financeValidation || (parsed.financingConfirmed ? 'confirmed' : 'absent'),
          financingSource: parsed.financingSource || null,
  ...
  async function runGasUi(config) {
    ...
    if (contracts?.login) return await attemptAutoLogin('gas', config || {});
    if (contracts?.verification) return { state: 'needs_verification', provider: 'gas', stage: 'verification_required', message: 'Gases del Caribe requiere completar una verificación visible antes de consultar.', results: [] };
    if (!contracts || !contracts.length) return recoverableSessionOutcome('gas', config, 'parse_contracts', 'Gases del Caribe no confirmó los contratos en la sesión actual. Se recargará el portal y se intentará recuperar el acceso.', {
      contractCount: 0,
  ...
  function mergeUiDebt(primary, fallback) {
    ...
    const merged = primaryResults.map(record => {
      ...
      // Invoice counters and installment metadata come from provider
      // endpoints/cards that explicitly expose those fields. They enrich the
      // UI totals without replacing the human-visible monthly/total debt.
      ['facturasTotales', 'facturasPendientes', 'facturasVencidas', 'cuotaActual', 'cuotasTotales',
        'proximoPagoCOP', 'saldoPorFacturarGasCOP', 'saldoPorFacturarFinancieroCOP', 'financeValidation', 'financingSource'].forEach(key => {
        if (match[key] !== null && match[key] !== undefined && match[key] !== '') next[key] = match[key];
      });
      if (match.financingConfirmed) {
        next.deudaConveniosCOP = match.deudaConveniosCOP ?? match.financiadaCOP ?? 0;
        next.financiadaCOP = match.financiadaCOP ?? next.deudaConveniosCOP;
        next.cuotaFinanciadaCOP = match.cuotaFinanciadaCOP ?? next.cuotaFinanciadaCOP ?? null;
        next.cuotaActual = match.cuotaActual ?? next.cuotaActual ?? null;
        next.cuotasTotales = match.cuotasTotales ?? next.cuotasTotales ?? null;
        next.financingConfirmed = true;
        if (Array.isArray(match.financiacion)) next.financiacion = match.financiacion;
      } else if (!next.financingConfirmed && match.financeValidation === 'absent' && match.deudaTotalCOP != null) {
        // A confirmed account with no financing card is an explicit $0, not
        // permission to retain a convention from an older synchronization.
        next.deudaConveniosCOP = 0;
        next.financiadaCOP = 0;
        next.cuotaFinanciadaCOP = null;
        next.cuotaActual = null;
        next.cuotasTotales = null;
        next.financiacion = [];
        next.financingConfirmed = false;
      }
  ...
  async function run(provider, config) {
    try {
      ...
      if (provider === 'water') {
        ...
        if (uiResult?.state === 'needs_verification') return uiResult;
        const apiResult = await runWater(config || {});
        if (uiResult?.results?.length && apiResult?.results?.length) return mergeUiDebt(uiResult, apiResult);
        return uiResult?.results?.length ? uiResult : apiResult;
      }
      if (provider === 'gas') {
