...
let mergePersistedUtilityRecord = null;
let onUtilityResultsPersisted = null;
...
function init(dbRef, saveFn, options = {}) {
  db = dbRef;
  saveData = saveFn;
  mergePersistedUtilityRecord = typeof options.mergeUtilityRecord === 'function'
    ? options.mergeUtilityRecord
    : null;
  onUtilityResultsPersisted = typeof options.onUtilityResultsPersisted === 'function'
    ? options.onUtilityResultsPersisted
    : null;
}
...
async function scrapeTripleAFromRenderedUi() {
  ...
  try {
    ...
    for (const policy of policies) {
      ...
      const month = apiSummary.deudaMesCOP ?? parsed.deudaMesCOP ?? parsed.deudaCOP;
      const financingConfirmed = apiSummary.financeValidation === 'confirmed'
        || apiSummary.financingConfirmed === true
        || (Array.isArray(apiSummary.financiacion) && apiSummary.financiacion.length > 0);
      const convention = financingConfirmed
        ? (apiSummary.deudaConveniosCOP ?? apiSummary.financiadaCOP ?? parsed.convenio ?? 0)
        : total === null || total === undefined ? null : 0;
      const checkedAt = new Date().toISOString();
      results.push({
        provider: 'Triple A', service: 'water', apartmentId: target.apartmentId, apartment: target.apartment,
        waterPaymentCode: policy.code, waterPaymentUrl: target.waterPaymentUrl || null,
        status: total > 0 ? 'pending' : parsed.status, deudaCOP: total, deudaMesCOP: month,
        deudaConveniosCOP: convention, deudaTotalCOP: total, deudaLabel: 'Deuda Total',
        numFacturas: apiSummary.numFacturas ?? (parsed.status === 'pending' ? 1 : 0),
        facturasTotales: apiSummary.facturasTotales ?? apiSummary.numFacturas ?? null,
        facturasPendientes: apiSummary.facturasPendientes ?? apiSummary.numFacturas ?? null,
        facturasVencidas: apiSummary.facturasVencidas ?? null,
        factura: apiSummary.factura || null, facturaValorCOP: apiSummary.facturaValorCOP ?? month,
        periodo: apiSummary.periodo || parsed.periodo || null, financiadaCOP: convention,
        cuotaFinanciadaCOP: financingConfirmed ? (apiSummary.cuotaFinanciadaCOP ?? null) : null,
        cuotaActual: financingConfirmed ? (apiSummary.cuotaActual ?? null) : null,
        cuotasTotales: financingConfirmed ? (apiSummary.cuotasTotales ?? null) : null,
        proximoPagoCOP: financingConfirmed ? (apiSummary.proximoPagoCOP ?? null) : null,
        saldoPorFacturarGasCOP: financingConfirmed ? (apiSummary.saldoPorFacturarGasCOP ?? null) : null,
        saldoPorFacturarFinancieroCOP: financingConfirmed ? (apiSummary.saldoPorFacturarFinancieroCOP ?? null) : null,
        financingConfirmed, financeValidation: financingConfirmed ? 'confirmed' : 'absent',
        financingSource: financingConfirmed ? (apiSummary.financingSource || null) : null,
        financiacion: financingConfirmed ? (apiSummary.financiacion || []) : [],
        facturas: Array.isArray(apiSummary.facturas) ? apiSummary.facturas : [],
        debtSource: apiSummary.debtSource || 'rendered_invoice', debtEndpointStatus: apiSummary.debtEndpointStatus ?? null,
        error: total === null || total === undefined ? (parsed.error || apiSummary.error || null) : null,
        checkedAt, scrapedAt: checkedAt,
      });
...
async function scrapeGasFromRenderedUi() {
  ...
  try {
    ...
    for (const contract of contracts) {
      ...
      const financingConfirmed = parsed.financeValidation === 'confirmed'
        || parsed.financingConfirmed === true
        || (Array.isArray(parsed.financiacion) && parsed.financiacion.length > 0);
      const convention = financingConfirmed
        ? (parsed.deudaConveniosCOP ?? parsed.financiadaCOP ?? 0)
        : total === null || total === undefined ? null : 0;
      const checkedAt = new Date().toISOString();
      results.push({
        provider: 'Gases del Caribe', service: 'gas', apartmentId: target.apartmentId, apartment: target.apartment,
        gasPaymentCode: contract.code, gasPaymentUrl: gasContractPaymentUrl(contract.code),
        status: total > 0 ? 'pending' : parsed.status, deudaCOP: total, deudaMesCOP: month,
        deudaConveniosCOP: convention, deudaTotalCOP: total, deudaLabel: 'Deuda Total',
        numFacturas: parsed.facturasPendientes ?? (parsed.status === 'pending' ? 1 : 0),
        facturasTotales: parsed.facturasTotales ?? null, facturasPendientes: parsed.facturasPendientes ?? null,
        facturasVencidas: parsed.facturasVencidas ?? null, factura: parsed.factura || null,
        periodo: parsed.periodo || null, facturaValorCOP: month, financiadaCOP: convention,
        cuotaFinanciadaCOP: financingConfirmed ? (parsed.cuotaFinanciadaCOP ?? null) : null,
        cuotaActual: financingConfirmed ? (parsed.cuotaActual ?? null) : null,
        cuotasTotales: financingConfirmed ? (parsed.cuotasTotales ?? null) : null,
        proximoPagoCOP: financingConfirmed ? (parsed.proximoPagoCOP ?? null) : null,
        saldoPorFacturarGasCOP: financingConfirmed ? (parsed.saldoPorFacturarGasCOP ?? null) : null,
        saldoPorFacturarFinancieroCOP: financingConfirmed ? (parsed.saldoPorFacturarFinancieroCOP ?? null) : null,
        financingConfirmed, financeValidation: financingConfirmed ? 'confirmed' : 'absent',
        financingSource: financingConfirmed ? (parsed.financingSource || null) : null,
        financiacion: financingConfirmed ? (parsed.financiacion || []) : [],
        debtSource: 'rendered_debt_cards', debtEndpointStatus: null,
        error: total === null || total === undefined ? (parsed.error || null) : null,
        checkedAt, scrapedAt: checkedAt,
      });
...
function portalFinancingSummary(payload) {
  // Current invoices and deferred balances share a response envelope. Only
  // read a convention when the field itself or its containing record proves
  // that it belongs to a financing card. This avoids copying generic `total`
  // or `saldoPorFacturar` values into every apartment.
  const records = portalObjectRecords(payload);
  const normalizeKey = value => normalizePortalText(value).replace(/ /g, '');
  ...
  const genericExplicit = across(['saldoPorFacturar'], record =>
  ...
  const progressFrom = record => {
    const raw = directField(record, [
      'cuotasFacturadas', 'cuotasPagadas', 'billedQuotas', 'paidQuotas',
      'quotaProgress', 'installmentProgress', 'cuotas', 'quotas',
    ]);
...
function tripleARecord(target, subscription, checkedAt = new Date().toISOString(), details = {}) {
  ...
  const financeConfirmed = details.financeValidation === 'confirmed'
    || details.financingConfirmed === true
    || (Array.isArray(details.financiacion) && details.financiacion.length > 0);
  const convention = financeConfirmed
    ? (details.deudaConveniosCOP ?? details.financiadaCOP ?? 0)
    : details.deudaTotalCOP !== undefined ? 0 : null;
  ...
  return {
    ...
    financiadaCOP: convention,
    cuotaFinanciadaCOP: financeConfirmed ? (details.cuotaFinanciadaCOP ?? null) : null,
    cuotaActual: financeConfirmed ? (details.cuotaActual ?? null) : null,
    cuotasTotales: financeConfirmed ? (details.cuotasTotales ?? null) : null,
    proximoPagoCOP: financeConfirmed ? (details.proximoPagoCOP ?? null) : null,
    saldoPorFacturarGasCOP: financeConfirmed ? (details.saldoPorFacturarGasCOP ?? null) : null,
    saldoPorFacturarFinancieroCOP: financeConfirmed ? (details.saldoPorFacturarFinancieroCOP ?? null) : null,
    financingConfirmed: financeConfirmed,
    financeValidation: financeConfirmed ? 'confirmed' : 'absent',
    financingSource: financeConfirmed ? (details.financingSource || null) : null,
    financiacion: financeConfirmed ? (details.financiacion || []) : [],
...
async function fetchTripleAPortalSummary(page, subscription, authHeader) {
  ...
  const debtFinancing = portalFinancingSummary(debtPayload);
  const financing = financingPayloads.length
    ? portalFinancingSummary(financingPayloads)
...
function gasRecord(target, parsed, checkedAt = new Date().toISOString()) {
  ...
  const financeConfirmed = parsed.financeValidation === 'confirmed'
    || parsed.financingConfirmed === true
    || (Array.isArray(parsed.financiacion) && parsed.financiacion.length > 0);
  const convenio = financeConfirmed
    ? (parsed.deudaConveniosCOP ?? parsed.financiadaCOP ?? 0)
    : parsed.deudaTotalCOP !== undefined || parsed.deudaCOP !== undefined ? 0 : null;
  ...
  return {
    ...
    financiadaCOP: convenio,
    cuotaFinanciadaCOP: financeConfirmed ? (parsed.cuotaFinanciadaCOP ?? null) : null,
    cuotaActual: financeConfirmed ? (parsed.cuotaActual ?? null) : null,
    cuotasTotales: financeConfirmed ? (parsed.cuotasTotales ?? null) : null,
    proximoPagoCOP: financeConfirmed ? (parsed.proximoPagoCOP ?? null) : null,
    saldoPorFacturarGasCOP: financeConfirmed ? (parsed.saldoPorFacturarGasCOP ?? null) : null,
    saldoPorFacturarFinancieroCOP: financeConfirmed ? (parsed.saldoPorFacturarFinancieroCOP ?? null) : null,
    financingConfirmed: financeConfirmed,
    financeValidation: financeConfirmed ? 'confirmed' : 'absent',
    financingSource: financeConfirmed ? (parsed.financingSource || null) : null,
    financiacion: financeConfirmed ? (parsed.financiacion || []) : [],
...
function gasDebtSummary(payload) {
  ...
  const convenio = financingConfirmed
    ? (financing.financiadaCOP ?? 0)
    : baseTotal === null && month === null ? null : 0;
  const total = baseTotal ?? (
    financingConfirmed && month !== null && convenio !== null
      ? month + convenio
      : month
  );
  return {
...
async function fetchGasDebtSummary(page, contractId, authHeader) {
  ...
  return {
    ...
    cuotaFinanciadaCOP: summary.financing.financeValidation === 'confirmed' ? summary.financing.cuotaFinanciadaCOP : null,
    cuotaActual: summary.financing.financeValidation === 'confirmed' ? summary.financing.cuotaActual : null,
    cuotasTotales: summary.financing.financeValidation === 'confirmed' ? summary.financing.cuotasTotales : null,
    proximoPagoCOP: summary.financing.financeValidation === 'confirmed' ? summary.financing.proximoPagoCOP : null,
    saldoPorFacturarGasCOP: summary.financing.financeValidation === 'confirmed' ? summary.financing.saldoPorFacturarGasCOP : null,
    saldoPorFacturarFinancieroCOP: summary.financing.financeValidation === 'confirmed' ? summary.financing.saldoPorFacturarFinancieroCOP : null,
    financingConfirmed: summary.financing.financeValidation === 'confirmed',
    financeValidation: summary.financing.financeValidation,
    financingSource: summary.financing.financingSource || null,
    financiacion: summary.financing.financeValidation === 'confirmed' ? summary.financing.financiacion : [],
...
function aggregateAirEInvoices(items) {
  ...
  return Object.fromEntries(Object.entries(grouped).map(([nic, group]) => {
    ...
    return [nic, {
      ...
      financiacion: [],
      facturasTotales: group.invoices.length,
...
function runScrapeOnce(reason) {
  ...
  airEScrapePromise = enqueueServiceBrowserRun(async () => {
    ...
    try {
      ...
      if (db && saveData && results.length) persistResults(results, reason);
      return results;
    } catch (e) {
...
function utilityResultIsConfirmed(result) {
  return ['pending', 'paid'].includes(String(result?.status || '').toLowerCase())
    && (result?.deudaTotalCOP !== null && result?.deudaTotalCOP !== undefined
      || result?.deudaCOP !== null && result?.deudaCOP !== undefined);
}

function notifyPersistedUtilityChanges(provider, results, changes, reason) {
  if (!onUtilityResultsPersisted || !changes.length) return;
  Promise.resolve(onUtilityResultsPersisted({ provider, results, changes, reason }))
    .catch(error => console.error(`[SERVICES] ${provider} payment-change notification error:`, error.message));
}

function persistUtilityResults(results, { provider, match, reason = 'scheduler' } = {}) {
  if (!db || !results?.length) return { persisted: 0, changes: [] };
  if (!Array.isArray(db.utilityRecords)) db.utilityRecords = [];
  const persistedResults = [];
  const changes = [];

  for (const result of results) {
    const normalized = { ...result, provider: provider || result.provider };
    const idx = db.utilityRecords.findIndex(record => match(record, normalized));
    const existing = idx >= 0 ? db.utilityRecords[idx] : null;
    const merged = mergePersistedUtilityRecord
      ? mergePersistedUtilityRecord(existing, normalized)
      : { ...(existing || {}), ...normalized };
    if (mergePersistedUtilityRecord && utilityResultIsConfirmed(normalized)
        && merged?.paymentChange && ['partial_payment', 'full_payment'].includes(merged.paymentChange.status)) {
      changes.push({
        provider: normalized.provider,
        service: normalized.service || null,
        apartmentId: normalized.apartmentId ?? existing?.apartmentId ?? null,
        apartmentName: normalized.apartment || existing?.apartment || null,
    ...
    if (idx >= 0) db.utilityRecords[idx] = merged;
    else db.utilityRecords.push(merged);
    persistedResults.push(merged);
  }
  saveData?.();
  notifyPersistedUtilityChanges(provider, persistedResults, changes, reason);
  return { persisted: persistedResults.length, changes };
}

function persistWaterResults(results, reason = 'scheduler') {
  const outcome = persistUtilityResults(results, {
    provider: 'Triple A', reason,
    match: (record, result) => record.provider === 'Triple A' && record.service === 'water' &&
      (Number(record.apartmentId) === Number(result.apartmentId) || record.apartment === result.apartment),
  });
  if (outcome.persisted) console.log(`[SERVICES] Persisted ${outcome.persisted} Triple A water record(s).`);
  return outcome;
}
...
function runWaterScrapeOnce(reason) {
  ...
  waterScrapePromise = enqueueServiceBrowserRun(async () => {
    ...
    try {
      ...
      if (db && saveData && results.length) persistWaterResults(results, reason);
      return results;
    } catch (e) {
...
function persistGasResults(results, reason = 'scheduler') {
  const outcome = persistUtilityResults(results, {
    provider: 'Gases del Caribe', reason,
    match: (record, result) => record.provider === 'Gases del Caribe' && record.service === 'gas' &&
      (Number(record.apartmentId) === Number(result.apartmentId) || record.apartment === result.apartment),
  });
  if (outcome.persisted) console.log(`[SERVICES] Persisted ${outcome.persisted} Gases del Caribe record(s).`);
  return outcome;
}
...
function runGasScrapeOnce(reason) {
  ...
  gasScrapePromise = enqueueServiceBrowserRun(async () => {
    ...
    try {
      ...
      if (db && saveData && results.length) persistGasResults(results, reason);
      return results;
    } catch (error) {
...
function persistResults(results, reason = 'scheduler') {
  const outcome = persistUtilityResults(results, {
    provider: 'Air-e', reason,
    match: (record, result) => {
      if (record.provider !== 'Air-e') return false;
      const sameApartmentId = result.apartmentId !== null && result.apartmentId !== undefined &&
        record.apartmentId !== null && record.apartmentId !== undefined &&
        Number(record.apartmentId) === Number(result.apartmentId);
      const sameApartmentName = String(result.apartment || '').trim() &&
        String(record.apartment || '').trim() === String(result.apartment || '').trim();
      if (sameApartmentId || sameApartmentName) return true;
      return !result.apartmentId && !result.apartment && !record.apartmentId && !record.apartment &&
        record.nic === result.nic;
    },
  });
  if (outcome.persisted) console.log(`[SERVICES] Persisted ${outcome.persisted} Air-e records.`);
  return outcome;
}
